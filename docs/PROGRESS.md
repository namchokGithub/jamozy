...
All 4 `docs/DOMAIN-MODEL.md` open questions resolved (EXP/Level derived, settings on user doc, Leitner-box spaced repetition, sequential unlock — [[DEC-006]]–[[DEC-009]]). Added `domain/models/user-profile.ts` (`levelFromExp`), extended `domain/models/review-item.ts` (`box`, `nextReviewAt`, `nextBox`, `nextReviewDate`), updated `ReviewRepository`/`FirebaseReviewRepository` (`markResolved` → `updateReviewItem`). `pnpm build`/`lint`/`vitest run` all pass (9 tests). Remaining optional: add a Web App to the `jamozy` project for `messagingSenderId`/`appId`. Next real step: build `application/` use cases (`get-course`, `get-lesson`, `complete-lesson`, `update-progress`, `get-review-items`) and seed sample course/unit/lesson content to actually exercise the repositories.

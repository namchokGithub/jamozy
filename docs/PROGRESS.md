...
| Domain/application/infrastructure folder skeleton | In progress | `domain/models` + `domain/repositories` + `infrastructure/firebase/{firebase.ts,repositories,mappers}` done (see [[DEC-002]]); `application/` and `features/` still empty — no longer blocked on open decisions, all 4 resolved ([[DEC-006]]–[[DEC-009]]) |
...
`docs/requirement.md` (detailed product spec) cross-checked against `docs/DOMAIN-MODEL.md`/`docs/DECISIONS.md`. 3 conflicts reaffirmed as originally decided (spaced repetition, flat EXP/Level formula, 3-state lesson status — [[DEC-006]], [[DEC-008]], [[DEC-009]]); 4 schema gaps closed: `LessonExercise.difficulty`/`meaningTh`/`meaningEn` ([[DEC-010]]), `UserStats` entity ([[DEC-011]]), `ReviewItem.reason` ([[DEC-012]]), full `UserSettings` (7 fields, replacing the old 2-field placeholder — [[DEC-013]]). `pnpm build`/`lint`/`vitest run` all pass. `docs/requirement.md` itself left unedited — `docs/DECISIONS.md`/`docs/DOMAIN-MODEL.md` are authoritative where they disagree.

Remaining optional: add a Web App to the `jamozy` Firebase project for `messagingSenderId`/`appId`. Next real step: build `application/` use cases (`get-course`, `get-lesson`, `complete-lesson`, `update-progress`, `get-review-items`) and seed sample course/unit/lesson content to actually exercise the repositories.
